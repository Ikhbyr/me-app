...
final class GoogleAuthenticator implements GoogleAuthenticatorInterface
{
    ...
    public function __construct(int $passCodeLength = 6, int $secretLength = 20, ?Carbon $instanceTime = null, int $codePeriod = 30)
    {
        /*
        ...
        $this->instanceTime = $instanceTime ?? new Carbon();
    }
    ...
    public function checkCode($secret, $code, $discrepancy = 1): bool
    {
        ...
        for ($i = -$discrepancy; $i < $periods + $discrepancy; ++$i) {
            $dateTime = new Carbon('@'.($this->instanceTime->getTimestamp() - ($i * $this->periodSize)));
     ...
     *
     * @param string                                   $secret
     * @param float|string|int|\Carbon|null $time
     */
    public function getCode($secret, /* \Carbon */ $time = null): string
    {
        if (null === $time) {
        ...
        if ($time instanceof Carbon) {
            ...
            @trigger_error(
                'Passing anything other than null or a Carbon to $time is deprecated as of 2.0 '.
     ...
     * @param string $secret
     *
     * @deprecated deprecated as of 2.1 and will be removed in 3.0. Use App\Services\GoogleAuthenticator\GoogleQrUrl::generate() instead.
     */
    public function getUrl($user, $hostname, $secret): string
    {
        @trigger_error(sprintf(
            'Using %s() is deprecated as of 2.1 and will be removed in 3.0. '.
            'Use App\Services\GoogleAuthenticator\GoogleQrUrl::generate() instead.',
            __METHOD__
        ), \E_USER_DEPRECATED);
...
class_alias('App\Services\GoogleAuthenticator\GoogleAuthenticator', 'Google\Authenticator\GoogleAuthenticator', false);
